...
**Last updated:** 2026-09-22 (rev 9 — `AccountSummary` (§7.5) gains `category`, mirroring `AccountPayload.category`, so the M3 sidebar can group by category and pin favorites without a full-record fetch; no ADR, no migration — `AccountSummary` is derived, never sealed; rev 8 — `AccountPayload` gains `revisionHistory` and `passwordHistory` / `passwordChangedAt` are now written by `VaultRepository` on every edit (§7.2, §7.3, §7.7); a new optional payload key, absorbed by the `unknown` forward-compat bag — no ADR, no migration, no schema bump; §4 app-source folder is `PassFort/` not `App/` (Xcode's synchronized-group folder; `.xcodeproj` still at the repo root) and its `Info.plist` is build-generated; open decision 11 re-scoped ("last used" sidecar deferred past M3 — the GUI list has no recent sort); rev 7 — recovery-key DEK slot folded into header format v1 per ADR-0007: §5.3 gains `slot_count` + an optional second slot, no version bump, no migration; §5.6 recovery key rendered Crockford Base32; open decisions 5, 6, 7, 11, 17 resolved (JSON payload, GRDB, 256-byte padding, `usedAt` deferred, recovery slot in v1); rev 6 — Botan pin bumped 3.12.0 → 3.13.0 per ADR-0001 amendment; rev 5 — CI and release pipeline added as §13.2–§13.4, §13 retitled "Testing, CI, and release", open decision 16 added; rev 4 — Azure backend §10 and web client §11 folded in per ADR-0005/0006, former §10–§13 renumbered to §12–§15)
...
struct AccountSummary: Sendable, Identifiable {   // the unlocked in-memory index of §3.4 / §8.3
    ...
    var tags: [String]
    var favorite: Bool
    var category: AccountCategory   // mirrors AccountPayload.category (§7.3) — drives sidebar grouping
    var isConflict: Bool
    var isDeleted: Bool
}
...
11. **"Last used" storage.** → **rev 7:** M2 leaves `usedAt` `nil` and never writes it; "last used" becomes a local-only sidecar when a client first needs it. → **rev 8 (M3):** still not needed — the GUI list sorts by title, no recent/frequent sort. Re-scoped as "build the sidecar (schema-v2 or `usage.sqlite`, never the `usedAt` payload field) if and when a recent/frequent sort is added, M4+". The payload field stays for forward-compat. (§7.2)
